extern crate ssim;
use ssim::sim::{ Simulation, TerminationCondition };
use ssim::cli;
use ssim::math::{ Vec3 };
use ssim::constants::*;
...
fn main() {
    ...
    sim.set_termination_condition(TerminationCondition::ElapsedTime(60.0));
    ...
    let output_device = OutputDevice::from_cli_config(&sim, &cli_matches);
    sim.set_output_device(output_device);
    sim.run();
}
...
// 2. [x] Improve OutputDevice, move it into its own file
// 
// Mar 19th
